      ...
      <div className="flash-sale-track">
        {items.map((item) => {
          ...
          const progress = Math.min(
            100,
            Math.round((item.soldCount / safeStock) * 100),
          );
          ...
          return (
            <article key={item.id} className="flash-card">
              ...
              <Link
                to={`/product/${item.id}`}
                className="flash-card-image-wrap"
                style={{ display: 'block' }}
              >
                {item.image ? (
                  <img
                ...
                {discountPercent > 0 && (
                  <div className="flash-badge-discount">
                    <span className="flash-badge-discount-num">
                      {discountPercent}%
                    </span>
                ...
                {/* Quick-add icon (bottom-right, hover only) */}
                <button
                  type="button"
              ...
              <div className="flash-card-body">
                ...
                <div className="flash-card-prices">
                  <span className="flash-card-price-current">
                    {formatVnd(item.price)}
                  </span>
                  {typeof item.originalPrice === 'number' &&
                    item.originalPrice > item.price && (
                      <span className="flash-card-price-original">
                ...
                <div className="flash-progress-wrap">
                  ...
                  <span
                    className={`flash-progress-text${progress < 50 ? ' low' : ''}`}
                  >
                    {progress >= 100
                      ? 'Hết hàng'
                      : isAlmostOut
